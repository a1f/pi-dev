...
# Pinned pi version — bump deliberately. `@latest` would re-pull whatever npm
# serves on every rebuild (the spawn.sh `--build` forces a rebuild check on
# every run), exposing this image to any malicious release in the channel.
# Check current: `npm view @earendil-works/pi-coding-agent version`.
...
# Non-root user. On macOS the host GID is often 20, which on Debian is the
# pre-existing `dialout` group — leaving pi in dialout is surprising (`id pi`
# shows dialout membership and any future image with /dev/tty access would
# inherit that group). Force a clean `pi` group name at the host GID by
# renaming dialout out of the way if needed. `gosu` switches to this user in
# the entrypoint — we don't `USER pi` here because the entrypoint needs to
# start as root to apply firewall rules.
RUN if getent group "${GID}" >/dev/null; then \
        existing=$(getent group "${GID}" | cut -d: -f1) ; \
        if [ "$existing" != "pi" ]; then \
            groupmod -n pi "$existing" ; \
        fi ; \
    else \
        groupadd -g "${GID}" pi ; \
    fi \
 && useradd -m -u "${UID}" -g pi -s /bin/bash pi \
